...
namespace SkyDriveNet
{
    public partial class SkyDriveClient
    {
        ...
        /// <returns></returns>
        public MetaData GetFolder(string folder)
        {
            List<MetaData> files = null;

            var request = _requestHelper.CreateFolderRequest(folder);
            var metaData = Execute<MetaData>(ApiType.Base, request);

            // simple directory with ID
            request = _requestHelper.CreateFolderFilesRequest(folder);
            var contents = Execute<MetaDataCollection>(ApiType.Base, request);
            if (contents != null)
                metaData.Files = contents.data;
